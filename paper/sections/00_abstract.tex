...
Evaluation is currently conducted in simulation using a reproducible
synthetic-pass pipeline, Monte Carlo diagnostics, and CRLB sensitivity
analysis.  A trace-driven hardware-in-the-loop (HIL) validation plan is
described using an LR1121/STM32 packet source and a USRP B210 IQ capture
front-end.  No real satellite over-the-air validation is claimed.
Meter-level localization accuracy and GNSS replacement are explicitly outside
the scope of the current work.  Rather than replacing GNSS, LEO-DTF is
intended as a complementary coarse localization and uplink state-estimation
primitive for bounded-region D2S IoT scenarios.
\end{abstract}