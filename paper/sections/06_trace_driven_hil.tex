...
\begin{itemize}
\item \textbf{LR1121/STM32 packet source}: The Semtech LR1121 transceiver~\cite{semtech_lr1121_datasheet}
        is configured in transmit-only mode (LR-FHSS or LoRa~\cite{semtech_lr_fhss_phy})
        using an STM32 host microcontroller.  The LR1121 does \emph{not} receive; it serves
        solely as a characterizable packet waveform source with known timing.
  \item \textbf{USRP B210}~\cite{ettus_usrp_b210}: A Software-Defined Radio (SDR) front-end captures
...
Engineering details for the hardware setup, RF safety, SDR capture protocol,
and LR1121/STM32 packet source configuration are maintained in
\texttt{docs/hardware/}.  The design documents are:
\begin{itemize}
  \item \texttt{docs/hardware/rf\_safety\_checklist.md} --- RF safety and
        no-transmit checklist.
  \item \texttt{docs/hardware/usrp\_b210\_capture\_protocol.md} --- USRP B210
        capture metadata requirements and post-capture analysis flow.
  \item \texttt{docs/hardware/lr1121\_stm32\_packet\_source.md} --- LR1121/STM32
        configuration and logging format.
  \item \texttt{docs/hardware/trace\_driven\_validation\_checklist.md} --- per-run
        checklist for validating the pipeline from IQ capture to posterior.
\end{itemize}