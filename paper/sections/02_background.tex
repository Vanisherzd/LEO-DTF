...
\cite{kodheli_nb_iot_2021}.  Devices communicating directly with LEO satellites
...
\subsection{Doppler-Aided Localization and State Estimation}
LEO Doppler has been studied as a positioning source in several prior
lines of work.  Early approaches used single-epoch Doppler measurements
from multiple satellite passes for rough geolocation
\cite{liu_geometric_2026}; these methods require many passes and
provide accuracy limited by satellite geometry.  More recent work in NTN
positioning considers joint Doppler-DMIMO techniques and multi-cell
uplink approaches that benefit from network-side cooperation
\cite{dureppagari_leo_2024}.  Oscillator imperfections in low-cost IoT
devices are identified as a major practical barrier, with CFO and drift
needing explicit estimation \cite{ettefagh_integrated_2025}.

LEO-DTF differs from these lines of work by focusing on bounded-ROI
coarse localization and uplink state estimation using a nuisance-aware
Doppler-time posterior in a reproducible synthetic and trace-driven
evaluation setting.  Rather than targeting global geolocation or
GNSS-replacement accuracy, it seeks to narrow the candidate region
for D2S IoT scheduling and link adaptation within a known coverage area.
...
trajectory is tracked over an extended pass \cite{liu_geometric_2026}.
...
%% Bibliography notes -- real references used above
%% References are now replaced with verified arXiv/IEEE/3GPP citations.