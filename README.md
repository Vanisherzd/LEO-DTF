...
| `scripts/export_posterior_diagnostic.py` | Export posterior grid heatmap from estimator (synthetic scenario) |
| `scripts/check_paper_sanity.py` | Check LaTeX structural sanity (no pdflatex required) |
| `scripts/list_placeholder_citations.py` | Report placeholder citations in paper/refs.bib |
...
- **Figures and tables:** generated by scripts from synthetic results
  ...
  `run_ambiguity_ablation.py`).  The posterior heatmap is now a genuine
  synthetic diagnostic produced by `export_posterior_diagnostic.py` when run
  (`make_paper_figures.py` then renders it).  A toy Gaussian fallback
  (`posterior_heatmap_placeholder.pdf`) is used only when the diagnostic
  script has not been executed; all other figures are from synthetic data.
